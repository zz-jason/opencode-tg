...
**组件说明**：
1. **Telegram Bot**：Python 程序，使用 `python-telegram-bot` 库，配置 HTTP 代理连接 Telegram API。
2. **OpenCode Client**：封装 OpenCode OpenAPI 的 HTTP 客户端，支持会话管理、消息发送、事件订阅等。
3. **会话管理器**：管理用户与会话的映射关系（内存或 SQLite 存储）。
4. **命令处理器**：解析用户命令，调用相应的 OpenCode API。
5. **消息流处理器**：处理 OpenCode 的流式响应，实时推送至 Telegram。
